...
async def ocr_endpoint(file: UploadFile = File(...)):
    try:
        ...
        return {"text": "N:50 P:30 K:20"}
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"OCR failed: {str(e)}")

@app.get("/get-npk-image/{crop}")
async def get_npk_image(crop: str, temp: float, humidity: float, ph: float, rainfall: float):
    try:
...
async def predict_crop_api(data: SoilData):
    try:
        ...
        return {"prediction": prediction, "npk": npk, "image": output_image, "probabilities": probabilities, "model_accuracy": model_accuracy, "metrics": metrics, "comparison_metrics": comparison_metrics}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
